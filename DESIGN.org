...
Further Addendum: This is actually just not feasible in general. We need some level of
unification in order to support CHR. See [[Unification, This Time With Feeling!]]
...
I think this system is dumb. I want to replace ~<-~ with a primitive "syntactic unification" constraint called ~===~, then a user-level ~==~ which has the rule ~(\=\= x x) <=> succeed~.
