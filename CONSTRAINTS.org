...
Before a constraint can be used, we must define the constraint and how it should be reified:
...
These read: "for any values ~x~, if there is a constraint in the store such that ~x~ must be symbol, then (respectively), if it is a ground non-symbol, fail; if it is a ground symbol, succeed".
